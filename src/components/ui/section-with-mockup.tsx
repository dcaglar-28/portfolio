...
export interface SectionWithMockupProps {
  ...
  /** Optional metadata (tags, links, year) rendered below the description */
...
export function SectionWithMockup({
  ...
  return (
    <section
      className={cn(
        "relative overflow-hidden bg-[#1a1d23] py-24 md:py-32",
        className,
      )}
    >
      <div className="relative z-10 mx-auto w-full max-w-[1220px] px-6 md:px-10">
        ...
        >
          <motion.div
            className={cn(
              "mx-auto mt-10 flex max-w-[546px] flex-col items-start gap-4 md:mt-0 md:mx-0",
              textOrderClass,
            )}
            variants={itemVariants}
          >
            <div className="space-y-2 md:space-y-1">
              <h3 className="text-3xl font-semibold leading-tight text-white md:text-[40px] md:leading-[53px]">
                {title}
              </h3>
            </div>

            <div className="text-sm leading-6 text-[#868f97] md:text-[15px]">
              {description}
            </div>
          ...
          >
            ...
            <motion.div
              className="relative z-10 h-[405px] w-full overflow-hidden rounded-[32px] border-0 bg-[#ffffff0a] backdrop-blur-[15px] md:h-[637px]"
              initial={{ y: 0 }}
              whileInView={{ y: reverseLayout ? 20 : 30 }}
              ...
              <div className="h-full p-0">
                <div className="relative h-full">
                  <div
                    ...
                    role="img"
                    aria-label={
                      typeof title === "string" ? `${title} project preview` : "Project preview"
                    }
                  />
                </div>
              </div>
